...
// Create flamethrower fire effect when client fire a flamethrower
...
// Stop or remove client flamethrower fire effect when client status changed
...
// Change client speed when client taking a flamethrower
    ...
    for (int client = 1; client <= MaxClients; client++)
    {
        if (IsClientInGame(client))
        {
            ...
            if (iFireEntity > MaxClients && IsValidEntity(iFireEntity) && IsClientFlamethrowerFireActive(client))
            {
                ...
                // Create self hurt, if player fire too close to the wall
                ...
                {
                    // if (!StrEqual(cEntityName, "ins_foliage", false)) // 用了TraceEntityFilter_OnlyWorld就不需要了
                    // {
